...
public final class CompositeUseContext {
    ...
    public static boolean handleSetBlock(Level level, BlockPos pos, BlockState state) {
        ...
        var current = entry.composite.parts().find(entry.partId).orElse(null);
        if (!belongsToPart(current == null ? null : current.state(), state)) return false;
        LAST_WRITE.set(entry);
        INTERNAL_WRITE.set(true);
        try {
            if (!keepsAttachedControl(current == null ? null : current.state(), state)) {
    ...
    static boolean belongsToPart(BlockState current, BlockState replacement) {
        return current != null && (replacement.isAir() || replacement.getBlock() == current.getBlock());
    }

    static boolean keepsAttachedControl(BlockState current, BlockState replacement) {
        return replacement.isAir() && current != null
                && (current.getBlock() instanceof net.minecraft.world.level.block.ButtonBlock
                || current.getBlock() instanceof net.minecraft.world.level.block.LeverBlock);
    }
