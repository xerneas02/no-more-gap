...
public final class NoMoreGapDebugCommand {
    ...
    private static com.mojang.brigadier.builder.LiteralArgumentBuilder<CommandSourceStack> root(String name) {
        return Commands.literal(name).requires(NoMoreGapDebugCommand::canDebug)
    ...
    private static boolean canDebug(CommandSourceStack source) {
        try {
            Object level = Commands.class.getField("LEVEL_GAMEMASTERS").get(null);
            for (var method : Commands.class.getMethods()) {
                if (method.getName().equals("hasPermission") && method.getParameterCount() == 1) {
                    return ((java.util.function.Predicate<CommandSourceStack>) method.invoke(null, level)).test(source);
                }
            }
